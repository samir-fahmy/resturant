...
from ai_core_sdk.ai_core_v2_client import PREFIX as prefix
from ai_core_sdk.ai_core_v2_client import get_home, CREDENTIAL_VALUES
from ai_core_sdk.credentials import get_nested_value
...
def load_service_key(service_key_json: str):
    ...
    for value in CREDENTIAL_VALUES:
        ...
        # resource_group is not a service key parameter
...
def get_profile_config_path(profile: str):
    ...
    if profile is not None:
        click.echo(f'Remember to set `{prefix}_PROFILE={profile.lower()}` to use your profile.')
    return config_path


def create_config_file(config_path: pathlib.Path,auth_url: str, client_id: str, client_secret: str, base_url: str, resource_group: str):
    config = create_config(auth_url=auth_url, client_id=client_id, client_secret=client_secret, base_url=base_url, resource_group=resource_group)
...
@click.pass_context
def configure(ctx, auth_url, client_secret, client_id, base_url, resource_group, service_key_json):
    ...
    client_secret = get_str_value('Please enter the client secret', service_key_json.get('client_secret', client_secret))
